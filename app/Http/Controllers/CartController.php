...
class CartController extends Controller
{
    ...
    public function save_cart(Request $request)
    {
        ...
        if(Session::has('cart') && !empty(Session::get('cart')))
       {
        ...
        //    dd($cart_data);
            foreach ($cart_data as $key => $value) 
            {
                ...
                if((intval($value['product_id']) == intval($request['product_id'])) && $request['type'] == 1)
                {
                    $flag = 1;
                    $rowid = $value['cart_id'];
                    // dd($rowid);
                    $cart_data[$rowid]['quantity_selected'] += $request['quantity'];
                    $cart_data[$rowid]['sub_total'] = $cart_data[$rowid]['price']*$cart_data[$rowid]['quantity_selected'];                    
                ...
                if((intval($value['product_id']) == intval($request['product_id'])) && $request['type'] == 2)
                {
                    $flag = 1;
                    $rowid = $value['cart_id'];
                    // dd($rowid);
                    ...
                    Session::forget($rowid);
                    Session::put('cart',$cart_data); 
                    return redirect()->route('cart')->with('notify_success','Merchandise Added To Cart Successfully!');
                    // return response()->json(['msg' => 'Product Added to cart Successfully', 'status' => 1]);
                }
       ...
       if($request['type'] == 1)
       {
        ...
        if($product_id != "" && intval($qty) > 0)
        {
            ...
            $cart[$cartId]['weight_kg'] = $product->weight_kg;
            $cart[$cartId]['weight_pound'] = $product->weight_pound;
            $cart[$cartId]['height'] =  $product->height;
            $cart[$cartId]['depth'] =  $product->depth;
            $cart[$cartId]['width'] =  $product->width;
            $cart[$cartId]['price'] = $product->price;
            $cart[$cartId]['sub_total'] = floatval($product->price*$qty);
    ...
    elseif ($request['type'] == 2) {
        ...
        if($product_id != "" && intval($qty) > 0)
        {
            ...
            $product = Merchandise::where('id',$product_id)->with('merchandiseHasMainImage')->first();
             //dd($product);
            $cart[$cartId]['cart_id'] = $cartId;
            $cart[$cartId]['stock_qty'] = $product->qty;
            $cart[$cartId]['product_id'] = $product_id;
            ...
            $cart[$cartId]['price'] = $product->price;
            $cart[$cartId]['sub_total'] = floatval($product->price*$qty);
            $cart[$cartId]['image'] = !empty($product->merchandiseHasMainImage[0]) ? $product->merchandiseHasMainImage[0]->img_path : asset('images/noimg.png');

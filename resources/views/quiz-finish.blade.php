...
@section('content')
 ...
 <section class="logout-sec">
       ...
       <div class="maincolumn">
        @if(intval($quiz->percentage) >= 70)
        <div class="course-list">
               <p class="headline mc-b-2 mc-t-2">Hooray! you have passed the test. Your score is {{$quiz->percentage}}%</p>
               <ul class="course-list">
                   <li><span>course name: </span> {{ucfirst($quiz->quizBelongsToCourse->title)}}</li>
                   <li><span>state: </span> {{$quiz->quizBelongsToCourse->courseBelongsToState->name}}</li>
                   <li><span> score: </span> {{$quiz->percentage}}% ({{$quiz->total_ques}} total questions {{$quiz->total_corr_ans}} correct)</li>
                   @if(Session::has('result_data'))
                   <li class="mc-t-2"> <a href="{{route('payafter',['course'=>$result_data['course_id']])}}" > click here to Pay For Certificate. </a></li>
                   @endif
                   <li class="mc-t-2"> <a href="{{route('quiz',['course'=>$quiz->course_id])}}" > click here to try the test again. </a></li>
                   <li class="mc-t-2"> <a href="{{route('courses')}}" > View Other Courses </a></li>
               </ul>
           </div>

        @else
           <div class="course-list">
               <p class="headline mc-b-2 mc-t-2">you did not pass the test. A score of 70% or higher is required
               </p>
               <ul class="course-list">
                   <li><span>course name: </span>  {{ucfirst($quiz->quizBelongsToCourse->title)}}</li>
                   <li><span>state: </span> {{$quiz->quizBelongsToCourse->courseBelongsToState->name}}</li>
           ...
           <!-- <div class="login-form">
               <form class="main-form mc-t-2">
                   <div class="row align-items-center">
                       ...
                       <div class="col-lg-4">
                           <button class="form-btn text-center"> search again</button>
                       </div>
                       </div>

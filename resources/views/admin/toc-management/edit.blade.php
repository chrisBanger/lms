        ...
        <div class="row align-items-center mc-b-3">
          <div class="col-lg-6 col-12">
            <div class="primary-heading color-dark">
              <h2>Edit Table Of Content</h2>
            </div>
          </div>
         
        </div>
       
        <form action="{{route('admin.savetoc')}}" method="POST" enctype="multipart/form-data" id="create_user_form" class="main-form create_user_form">
            @csrf
          ...
          <div class="row">
            ...
            <div class="col-lg-6 col-md-6 col-12">
              <div class="form-group">
               ...
               @foreach($courses as $course)
                ...
                <option {{$course->id == $toc->course_id ? 'selected' : ''}} value="{{$course->id}}">{{ucfirst($course->title)}}</option>
               @endforeach
                </select>
              </div>
            </div>
            ...
            <div class="col-lg-6 col-md-6 col-12">
              <div class="form-group">
                ...
                @if($toc->parent_toc != 0)
                <option value="{{$toc->parent_toc}}" selected="selected">{{$toc->tocBelongsToParentTOC->title}}</option>
                  @else
            ...
            <div class="col-lg-6 col-md-6 col-12">
              <div class="form-group">
                ...
                <input type="text" required name="title" value="{{$toc->title}}" placeholder="Enter Title"  class="form-control" required>
              </div>
            </div>
            <div class="col-lg-6 col-md-6 col-12">
              <div class="form-group">
                <label>Page Number*:</label>
                <input type="number" min="1" name="page_no" value="{{$toc->page_no}}"  class="form-control" required>
              </div>
            </div> 
            ...
            <div class="text-center">
              <button type="button" class="primary-btn primary-bg add-user">Save</button>
            </div>
          </div>
          </form>
          </div>
...
(()=>{
  @if($toc->parent_toc != 0)
  $('.parent-dd').removeAttr("style").show();
    ...
    $.ajax({
          ...
          success: function (msg) {
              ...
              if(msg.status == 1)
              {
                  ...
                  $('.parent-dd').empty().append('<option selected="selected" value="0">Select A Parent TOC</option>');
